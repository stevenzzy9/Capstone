...
public class EventFeedDemo {
	 ...
	 *             Error communicating with the server.
	 */
	public static ArrayList<GoogleCalendarModel> dateRangeQuery(String usergooglename,String userGooglePassword ,
			CalendarService myService) throws ServiceException, IOException {

		
		// Create the necessary URL objects.
		try {
			metafeedUrl = new URL(METAFEED_URL_BASE + usergooglename);
			eventFeedUrl = new URL(METAFEED_URL_BASE + usergooglename
		...
		myService.setUserCredentials(usergooglename, userGooglePassword);

		ArrayList<GoogleCalendarModel> googleCalendarList = new ArrayList<GoogleCalendarModel>();
		...
		for (int i = 0; i < resultFeed.getEntries().size(); i++) {
			...
			CalendarEventEntry entry = resultFeed.getEntries().get(i);
			GoogleCalendarModel tempCalendarModel = new GoogleCalendarModel();
			tempCalendarModel.setEventDesription(entry.getTitle()
					.getPlainText());
			tempCalendarModel.setEventTime(entry.getUpdated().toString());
			googleCalendarList.add(tempCalendarModel);
